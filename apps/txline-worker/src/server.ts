...
app.get('/txline/quizzes', async (req, res) => { try { res.json(await client.listQuizzes(req.query as Record<string, string>)); } catch (error) { res.status(502).json({error: error instanceof Error ? error.message : 'TXLINE_REQUEST_FAILED'}); }});

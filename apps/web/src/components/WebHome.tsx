        ...
        <div className="ticker-track">
          {[0,1].map((copyIndex)=>(
            <div className="ticker-group" key={copyIndex} aria-hidden={copyIndex===1||undefined}>
              {matchStats
                ?matchStats.stats.map((stat)=><span className="ticker-stat" key={`${copyIndex}-${stat.label}`} style={{position:'relative',paddingBottom:'8px'}}><small style={{fontSize:'10px',fontWeight:'700',letterSpacing:'1px'}}>{stat.label}</small><b style={{display:'flex',alignItems:'center',gap:'16px',justifyContent:'space-between',marginTop:'6px',padding:'8px 12px',borderRadius:'8px',background:'rgba(200,240,0,0.08)',border:'1px solid rgba(200,240,0,0.15)'}}><span style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'14px',fontWeight:'800'}}><TeamShield teamName={matchStats.home_team} size={22} promotional/><span style={{minWidth:'32px',textAlign:'right'}}>{stat.home}</span></span><span style={{opacity:0.3,fontSize:'12px',fontWeight:'300'}}>VS</span><span style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'14px',fontWeight:'800',flexDirection:'row-reverse'}}><span style={{minWidth:'32px',textAlign:'left'}}>{stat.away}</span><TeamShield teamName={matchStats.away_team} size={22} promotional/></span></b><i/></span>)
                :quizzes.length

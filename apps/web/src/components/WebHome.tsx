        ...
        <div className="ticker-track">
          {[0,1].map((copyIndex)=>(
            <div className="ticker-group" key={copyIndex} aria-hidden={copyIndex===1||undefined}>
              {matchStats
                ?matchStats.stats.map((stat)=><span className="ticker-stat" key={`${copyIndex}-${stat.label}`}><small style={{fontSize:'10px',fontWeight:'600',letterSpacing:'0.5px',opacity:0.8}}>{stat.label}</small><b style={{display:'flex',alignItems:'center',gap:'8px',justifyContent:'center',marginTop:'4px',fontSize:'13px',fontWeight:'700'}}><TeamShield teamName={matchStats.home_team} size={22}/><span>{stat.home}</span><span style={{opacity:0.5}}>·</span><span>{stat.away}</span><TeamShield teamName={matchStats.away_team} size={22}/></b><i/></span>)
                :quizzes.length
